...
const testimonialsTrigger =
    document.getElementById("testimonials-trigger");

const testimonialsPanel =
    document.getElementById("testimonials-panel");

const testimonialsList =
    document.getElementById("testimonials-list");

const testimonialsReturnButton =
    document.getElementById("testimonials-return-button");
...
const testimonials = [
    ...
    {
        quote:
            "Handed over a rough concept and got back a video that actually made people stop scrolling. Fast, sharp, no hand-holding needed.",
        author: "CA ANURAG SINGAL",
        role: "Partner at BetaFin Partners LLP",
        rating: 5
    },

    {
        quote:
            "It was an amazing experience working with magic plus. My whole linkedin profile got revamped in seconds.",
        author: "Nisha Bharti",
...
function openTestimonials() {
    ...
    testimonialsPanel.classList.remove(
        "hidden"
    );


    gsap.fromTo(
        testimonialsList.children,
        {
            opacity: 0,
            y: 16
        },
...
function closeTestimonials() {
    ...
    testimonialsPanel.classList.add(
        "hidden"
    );
...
let testimonialsAutoShown =
    false;


testimonialsTrigger.addEventListener(
    "click",
    openTestimonials
);


testimonialsReturnButton.addEventListener(
    "click",
    closeTestimonials
);
...
const worlds = [
...

    {
        ...
        slug: "linkedin-revamp",
        categorySlug: "linkedin-revamp",
        description:
            "Turning a dead profile into your best pitch.",

        status: "BUILDING THIS WORLD",

        focus: [
            "Banner, photo direction, headline and About section, rebuilt to actually get read.",
            "Positioning that makes recruiters and clients stop scrolling."
        ],
...
enterButton.addEventListener(
    ...
    () => {
        ...
        /* Show testimonials automatically,
           once, shortly after entering the
           universe — no click needed */

        setTimeout(
            () => {

                if (
                    testimonialsAutoShown
                ) return;
...
window.addEventListener(
    ...
    event => {
        ...
        if (
            ...
            if (
                !testimonialsPanel.classList.contains("hidden")
            ) {
...
function animate() {
    ...
    /* Core "heartbeat" — a slow breathing
       pulse plus a faster inner flicker,
       layered so it never feels mechanical */
    ...
    const breathe =
        Math.sin(coreTime * 0.6) *
        0.06;

    const flicker =
        Math.sin(coreTime * 3.1) *
        0.02;
